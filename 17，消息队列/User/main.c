...
#define  QUEUE_SIZE   sizeof(uint32_t) // 全局变量
  /* 队列中每个消息大小（字节） */
...
static void Receive_Task(void *pvParameters) {
    ...
    while (1) {
        /* 阻塞等待接收 */
        if (xQueueReceive(Test_Queue, &received_msg, portMAX_DELAY) == pdTRUE) {
            recv_cnt++;
            printf("RX[%d]: %s\n", recv_cnt, received_msg);
            
            if (recv_cnt >= 10) {
                /* 挂起发送任务（任务一） */
                vTaskSuspend(Send_Task_Handle);
                printf("已接收10次，发送任务挂起，接收任务即将删除\n");
                /* 删除自身（任务二） */
                vTaskDelete(NULL);
                /* 以下代码不再执行 */
            }
...
static void Send_Task(void *pvParameters) {
    const char *msg = "0123456789";   /* 字符串常量 */
    uint8_t send_cnt = 0;
    
    while (1) {
        if (send_cnt < 10) {
            /* 发送字符串指针（值拷贝） */
            if (xQueueSend(Test_Queue, &msg, 0) == pdPASS) {
                printf("TX[%d]: %s\n", send_cnt + 1, msg);
                send_cnt++;
            } else {
                printf("队列满，发送失败\n");
            ...
            /* 10次发送完毕，任务可进入空闲或挂起自身（但会被接收任务挂起） */
            printf("发送任务已完成10次发送\n");
            vTaskSuspend(NULL);   /* 挂起自身，等待被接收任务挂起前可能先执行到这里 */
        }
